class EventProcessor
  ...
  def process_push_event(event)
    ...
    Stats.current_stats.inc(:opened_issues)
  end
  ...
  def process_issues_event(event)
    ...
    if action == "opened"
      ...
      Stats.current_stats.inc(:closed_issues)
    end
  end
  ...
  def process_pull_request_event(event)
    ...
    action = event.data["action"]
    if action == "opened"
      Stats.current_stats.inc(:opened_pull_requests)
    elsif action == "closed"
      Stats.current_stats.inc(:closed_pull_requests)
    end
  end
